#!/usr/bin/env bash
#SBATCH --job-name=perpop_parent_filters_keepMono
#SBATCH --output=perpop_parent_filters_keepMono_%A_%a.out
#SBATCH --error=perpop_parent_filters_keepMono_%A_%a.err
...
# NEW output base (separate from your polymorphic-only version)
OUT_BASE="${IN_DIR}/filter_byPopulation/per_population_filters_keepMonomorphic"
...
' "$POP_MANIFEST" | while IFS=$'\t' read -r POP RP DONOR_AB DONOR_D; do
  ...
  if ! subset_two_cols "$IN_FILE" "$RP" "$DONOR" > "$SUBSET_FILE" 2>/dev/null; then
    {
      echo -e "chr\tpopulation\trp\tdonor\ttotal\tkept\tdrop_missing\tdrop_het\tdropped_total"
      echo -e "chr${CHR}\t${POP}\t${RP}\t${DONOR}\t0\t0\t0\t0\t0"
    } > "$SUM_FILE"
    echo "WARN: Missing parent column(s) for ${POP} on chr${CHR}: RP=${RP}, DONOR=${DONOR}" >&2
    continue
  fi

  # 2) Filter markers (drop if missing/het in either parent; KEEP monomorphic + polymorphic)
  {
    echo -e "Marker\tFailReasons" > "$FAIL_FILE"
    echo -e "Marker\tREF\tALT\t${RP}\t${DONOR}" > "$KEEP_FILE"
  }

  awk -F'\t' -v OFS='\t' -v CHR="chr'"$CHR"'" -v RP_NAME="$RP" -v DN_NAME="$DONOR" \
      -v FAIL_FILE="$FAIL_FILE" -v KEEP_FILE="$KEEP_FILE" '
    ...
    END{
      printf "chr\tpopulation\trp\tdonor\ttotal\tkept\tdrop_missing\tdrop_het\tdropped_total\n" > "'"$SUM_FILE"'"
      printf "%s\t%s\t%s\t%s\t%d\t%d\t%d\t%d\t%d\n",
             CHR, "'"$POP"'", RP_NAME, DN_NAME,
             total+0, kept+0, drop_miss+0, drop_het+0, dropped+0 >> "'"$SUM_FILE"'"
    }
  ' "$SUBSET_FILE"
